...
export function ShellBackdrop() {
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden">
      ...
      {/* the shell itself: large, soft-light, so it reads as texture not a lamp */}
      <img
        src={shellHero.url}
        alt=""
        aria-hidden
        className="absolute left-1/2 top-1/2 h-[95%] w-auto max-w-none -translate-x-1/2 -translate-y-1/2 -rotate-[8deg] select-none opacity-[0.26] mix-blend-soft-light [filter:grayscale(1)_brightness(1.25)_contrast(0.9)] [mask-image:radial-gradient(closest-side,black_38%,transparent_92%)] md:h-[150%] md:opacity-[0.3]"
      />
    </div>
  );
}
